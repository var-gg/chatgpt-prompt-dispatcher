...
- On some machines, long prompts can still fail closed with `PROMPT_VALIDATION_FAILED` if the composer never becomes visibly sendable. This is intentional: the strict path now prefers rejecting invisible/accessibility-only input over reporting a false success.
